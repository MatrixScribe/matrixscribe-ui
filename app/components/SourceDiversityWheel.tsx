...
export default function SourceDiversityWheel({ entity }: SourceDiversityWheelProps) {
  ...
  const segments = [
    { label: "Local", value: geo.local ?? 0, color: "var(--color-charcoal-mid)" },
    { label: "Regional", value: geo.regional ?? 0, color: "var(--color-sandstone-border)" },
    { label: "International", value: geo.international ?? 0, color: "var(--color-charcoal-light)" },
  ];

  const total = segments.reduce((sum, s) => sum + (typeof s.value === "number" ? s.value : 0), 0);
      ...
      <div className="relative w-full h-56 flex items-center justify-center">
        ...
        >
          {segments.map((s, i) => {
            ...
            const end = (cumulative + s.value) / safeTotal;
            cumulative += s.value;
